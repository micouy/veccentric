...
fn main() -> Result<(), pixels::Error> {
    ...
    // Draw state.
    let draw = |State { ref a, ref b, ref c }: &State, buffer: &mut Buffer| {
	...
	let calc_force = |body: &Vehicle, other_body: &Vehicle| {
		...
		// F = r_hat * G * M * m / |r|^2
		radius.normalize() * G * body.mass * other_body.mass / radius.mag_squared()
	};

    // Update state.
    let update = move |State { ref mut a, ref mut b, ref mut c }: &mut State, dt: f64| {
