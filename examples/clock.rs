...
fn main() -> Result<(), pixels::Error> {
    ...
    // Draw state.
    let draw = move |State { ref seconds, ref minutes, ref hours, .. }: &State, buffer: &mut Buffer| {
    ...
    let update = move |state: &mut State, dt: f64| {
		state.seconds = original_s.rotate(state.start.elapsed().as_secs_f64() * 2.0 * PI / 60.0);
		state.minutes = original_m.rotate(state.start.elapsed().as_secs_f64() * 2.0 * PI / 60.0_f64.powf(2.0));
		state.hours = original_h.rotate(state.start.elapsed().as_secs_f64() * 2.0 * PI / 60.0_f64.powf(3.0));
    };
